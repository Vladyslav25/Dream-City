...
namespace MeshGeneration
{
    public class MeshGenerator : MonoBehaviour
    {
        ...
        public static MeshFilter Create_CollisionGridMesh(Street _s, MeshFilter _mf, MeshRenderer _mr)
        {
            List<Cell> cells = _s.m_StreetCells.Values.ToList();
            Vector3[] vertices = new Vector3[cells.Count * 4];
            Mesh m = new Mesh();
            int maxRow = _s.m_RowAmount;
            m.subMeshCount = maxRow * GridManager.Instance.MaxGeneration * 2;
            for (int a = 0; a < cells.Count; a++)
            {
                Matrix4x4 rotationMatrix = Matrix4x4.Rotate(Quaternion.Inverse(cells[a].m_Orientation));
                for (int i = 0; i < 4; i++)
            ...
            for (int a = 0; a < cells.Count; a++)
            {
                ...
                Vector2Int pos = cells[a].Pos;
                int index = Mathf.Abs(pos.x) + pos.y * GridManager.Instance.MaxGeneration - 1;
                if (pos.x < 0) //if right Side
                    index += GridManager.Instance.MaxGeneration * maxRow;

                m.SetIndices(indices, MeshTopology.Triangles, index);
            }
